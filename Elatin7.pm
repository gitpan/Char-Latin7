...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Elatin7::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Elatin7::open;
    }
}
...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Elatin7::index($name, '::') == -1) && (Elatin7::index($name, "'") == -1)) {
            my $pkg;
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Elatin7::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Elatin7::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Elatin7::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Elatin7::uc($_[0]) =~ m{\A $pattern \z}xms };
...
This module is a run-time routines of the Latin7.pm.
Because the Latin7.pm automatically uses this module, you need not use directly.
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Elatin7::split returns
...
=item Chop string
  ...
  The Elatin7::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Elatin7::index($string,$substr,$position);
  $pos = Elatin7::index($string,$substr);
  ...
  the Elatin7::index function returns -1.

=item Reverse index string

  $pos = Elatin7::rindex($string,$substr,$position);
  $pos = Elatin7::rindex($string,$substr);

  Works just like Elatin7::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Elatin7::chr(65) is "A" in either ASCII or Latin-7, and
  Elatin7::chr(0x82a0) is a Latin-7 HIRAGANA LETTER A. For the reverse of Elatin7::chr,
  use Latin7::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Elatin7::binmode(FILEHANDLE, $disciplines);
  Elatin7::binmode(FILEHANDLE);
  Elatin7::binmode($filehandle, $disciplines);
  Elatin7::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Elatin7::open(FILEHANDLE, $mode, $expr);
  $rc = Elatin7::open(FILEHANDLE, $expr);
  $rc = Elatin7::open(FILEHANDLE);
  $rc = Elatin7::open(my $filehandle, $mode, $expr);
  $rc = Elatin7::open(my $filehandle, $expr);
  $rc = Elatin7::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Elatin7::open($fh, ">logfile")     # $fh is autovivified
  ...
  Elatin7::open my $fh, ">logfile" or die ...
  ...
  Elatin7::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Elatin7::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Elatin7::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Elatin7::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Elatin7::open(FH, "<:crlf:encoding(Latin7)", $path) or die "can't open $path: $!";
