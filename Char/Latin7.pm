...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Elatin7::uc($char[$i]) ne Char::Elatin7::lc($char[$i]))) {
            $char[$i] = '[' . Char::Elatin7::uc($char[$i]) . Char::Elatin7::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Latin7/Char::Latin7/JA.pm
...
Definitions in Char/Elatin7.pm.
  ...
  @{Char::Elatin7::eV}             qr{(?:[^\x0C\x0A\x0D])}
