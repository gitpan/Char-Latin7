...
# split
            ...
            my $e_string = 'Char::Elatin7::split';
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and (Char::Elatin7::uc($char[$i]) ne Char::Elatin7::lc($char[$i]))) {
...
and the cellular phone. This software treats Latin-7 directly. Therefor there is
not UTF8 flag.
...
=item * Possible to handle raw Latin-7 values

=item * Backward compatibility of data, script and how to

=item * Only Char/Latin7.pm and Char/Elatin7.pm, other modules are unnecessary
...
functions.
  ...
  ---------------------------------
  lc          Char::Elatin7::lc
  lcfirst     Char::Elatin7::lcfirst
  uc          Char::Elatin7::uc
  ucfirst     Char::Elatin7::ucfirst
  chr         Char::Elatin7::chr
  glob        Char::Elatin7::glob
  ---------------------------------
...
Char/Latin7.pm remove 'bytes::' at head of function name.
...
=head1 Perl5.6 Emulation on perl5.005
  ...
  --------------------------------------------------------------------
  Before          After                  in BEGIN { } of Char/Elatin7.pm
  --------------------------------------------------------------------
  binmode(...);   Char::Elatin7::binmode(...);   *CORE::GLOBAL::binmode = ...
  open(...);      Char::Elatin7::open(...);      *CORE::GLOBAL::open    = ...
  --------------------------------------------------------------------
...
=item * binmode (Perl5.6 emulation on perl5.005)
  ...
  * two arguments
  ...
  If you are using perl5.005, Char::Latin7 software emulate perl5.6's binmode function.
...
=item * open (Perl5.6 emulation on perl5.005)
  ...
  * three arguments
  ...
  If you are using perl5.005, Char::Latin7 software emulate perl5.6's open function.
  ...
  open(FH, "<:crlf:encoding(Char::Latin7)", $path) or die "can't open $path: $!";
...
=head1 Ignore utf8 pragma
  ...
  ---------------------------------------------------------------------
  use utf8;       # use utf8;            Char/Elatin7.pm provides utf8::*
  no utf8;        # no utf8;             functions even if 'no utf8;'
  use bytes;      # use bytes;           Char/Elatin7.pm provides bytes::*
...
page 402 of Programming Perl, 3rd ed. again.
  ...
  Information processing model beginning with this software
    ...
    +-----------------------------------+
    |      ASCII Compatible Encoding    | ex. Latin-7
    +-----------------------------------+
                (No UTF8 Flag)
...
I am thankful to all persons.
 ...
 http://search.cpan.org/dist/Latin-7-Regexp/
