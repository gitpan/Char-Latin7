...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Elatin7::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Elatin7::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  Scans a Latin-7 $string for delimiters that match pattern and splits the Latin-7
  ...
  pattern, so the delimiters may be of any size and need not be the same Latin-7
  $string on every match. If the pattern doesn't match at all, Char::Elatin7::split returns
  the original Latin-7 $string as a single substring. If it matches once, you get
  ...
  If Latin-7 $string is omitted, the function splits the $_ Latin-7 string.
...
=item Chop string
  ...
  Chops off the last character of a Latin-7 string contained in the variable (or
  Latin-7 strings in each element of a @list) and returns the character chopped.
  The Char::Elatin7::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Elatin7::index($string,$substr,$position);
  $pos = Char::Elatin7::index($string,$substr);

  Returns the position of the first occurrence of $substr in Latin-7 $string.
  The start, if specified, specifies the $position to start looking in the Latin-7
  $string. Positions are integer numbers based at 0. If the substring is not found,
  the Char::Elatin7::index function returns -1.

=item Reverse index string

  $pos = Char::Elatin7::rindex($string,$substr,$position);
  $pos = Char::Elatin7::rindex($string,$substr);

  Works just like Char::Elatin7::index except that it returns the position of the last
  occurence of $substr in Latin-7 $string (a reverse index). The function returns
  -1 if not found. $position, if specified, is the rightmost position that may be
  returned, i.e., how far in the Latin-7 string the function can search.
...
=item Lower case string
  ...
  Returns a lowercase version of Latin-7 string (or $_, if omitted). This is the
...
=item Lower case first character of string
  ...
  Returns a version of Latin-7 string (or $_, if omitted) with the first character
...
=item Upper case string
  ...
  Returns an uppercased version of Latin-7 string (or $_, if string is omitted).
...
=item Make character
  ...
  set. For example, Char::Elatin7::chr(65) is "A" in either ASCII or Latin-7, not Unicode,
  and Char::Elatin7::chr(0x82a0) is a Latin-7 HIRAGANA LETTER A. For the reverse of
  Char::Elatin7::chr, use Char::Latin7::ord.
