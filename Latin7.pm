...
my $qq_substr  = qr{(?: Latin7::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_getc;        # getc    to getc    or Latin7::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Latin7::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Latin7::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Latin7::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Latin7::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Latin7::re_t=%s; while($Latin7::re_t =~ %s){%s local $^W=0; local $Latin7::re_r=%s; %s$Latin7::re_t="$`$Latin7::re_r$'"; pos($Latin7::re_t)=length "$`$Latin7::re_r"; } return $Latin7::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Latin7::re_r=eval $Latin7::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Latin7::re_r=%s; %s"%s$Latin7::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Latin7::re_n=0; while(%s =~ %s){%s local $^W=0; local $Latin7::re_r=%s; %s%s="$`$Latin7::re_r$'"; pos(%s)=length "$`$Latin7::re_r"; $Latin7::re_n++} return %s$Latin7::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Latin7::re_r=%s; %s%s="%s$Latin7::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Latin7::re_r=%s; %s%s="%s$Latin7::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Latin7::re_r=eval $Latin7::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Elatin7.pm
works well.
